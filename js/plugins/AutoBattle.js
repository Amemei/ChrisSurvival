...
// ver1.03版に対して以下を追加修正
// スキル使用設定時の使用スキル処理を修正
// 装備武器タイプにより素殴りor最低MpCostスキルを選択
// 素殴り以外のスキルを使いたい武器タイプIDをRB_AB_WTYPE_TBLに追加すること
// 既存動作との切替をオプションに追加→一時的に封印
//                      by 龍尾(2019.6.23)
//
 ...
 * 
 * @param Auto Battle Eco Option Text
 * @desc オートバトル(Ecoモード)のオプション画面での表示名を変更します。
 * @default オートバトル(Ecoモード)
 * 
 * @help------------------------------------------------------
...
(function(){
	...
	// AutoBattle用武器タイプ配列(龍尾)
	var RB_AB_WTYPE_TBL = [2];
	...
	var autoBattleUsableMessage = String(parameters['Auto Battle Usable Message'] || '现在自动进行中 按shift解除');
	var autoBattleMessage = String(parameters['Auto Battle Message'] || '按Shift后自动进行事件');
	var autoBattleEcoOptionText = String(parameters['Auto Battle Eco Option Text'] || 'オートバトル(Ecoモード)');
...
console.log("ConfigManager.autoBattleEco:" + ConfigManager.autoBattleEco);
		this.clearActor();
	...
	Window_AutoBattle.prototype.initialize = function() {
		Window_Base.prototype.initialize.call(this,showX,showY,192,49);
...
Game_Actor.prototype.makeAutoBattleActions = function() {
    for (var i = 0; i < this.numActions(); i++) {
		...
		// オプションよりスキル使用時の行動切替(龍尾)
		if (ConfigManager.autoBattleEco) {
			// 装備中の武器タイプにより、使用スキルを変える
			var equips = this.equips();
			if (RB_AB_WTYPE_TBL.indexOf(equips[0].wtypeId) >= 0) {
				// 素殴り以外のスキルを使用したい武器を装備してる

				// Cost1以上のスキルが存在しない場合のために、素殴りセット
				this.setAction(i, list[0]);

				// Costが一番低い敵攻撃系スキルをセット
				var minCost = Number.MAX_VALUE;
				for(var j = 1; j < list.length; j++) {
					if (list[j].isSkill() && list[j].isDamage()) {
						var cost = this.skillMpCost(list[j].item());
						if (cost < minCost) {
							minCost = cost;
...
Game_Action.prototype.subject = function() {
	...
	// ConfigManager
	//-------------------------------------------------------------------------
	ConfigManager.autoBattleEco = true;

/*
	var RB_AB_ConfigManager_makeData = ConfigManager.makeData;
	ConfigManager.makeData = function() {
		var config = RB_AB_ConfigManager_makeData.call(this);
		config.autoBattleEco = this.autoBattleEco;
		return config;
	};

	var RB_AB_ConfigManager_applyData = ConfigManager.applyData;
	ConfigManager.applyData = function(config) {
		RB_AB_ConfigManager_applyData.call(this, config);
		this.autoBattleEco = this.readFlag(config, 'autoBattleEco');
	};

	//-------------------------------------------------------------------------
	// Window_Options
	//-------------------------------------------------------------------------
	var RB_AB_Window_Options_addGeneralOptions = Window_Options.prototype.addGeneralOptions;
	Window_Options.prototype.addGeneralOptions = function() {
		RB_AB_Window_Options_addGeneralOptions.call(this);
		this.addCommand(autoBattleEcoOptionText, 'autoBattleEco');
	};
	...
	if(!!PluginManager._parameters["AKUNOU_OptionDefault".toLowerCase()]) {
		...
		Window_Options.prototype.defaultAll = function() {
			...
			this.changeValue('autoBattleEco', false);
		};
	}
*/
